...
public class HeadLookController : MonoBehaviour {
    ...
    private float lookTime = 7.0f;
    private float tempTime;
    private Vector3 tempTarget;

	public Transform[] headLookTargets;
    ...
    public void LookAt()
    {
        if (lookTime < 0 && tempTarget == new Vector3(0, 0, 0))
        {
            LookAround();
          
        }

            if (lookTime < -tempTime)
            {
    ...
    {
        int d = Mathf.RoundToInt(Time.time);
        var random = new System.Random();

        var rand = random.Next(13);
        if (rand <= 2)
            rand = 2;
    ...
    {
        int d = Mathf.RoundToInt(Time.time);
		if(headLookTargets.Length > 0)
        {
			var random = Random.Range(0, headLookTargets.Length);

			target = headLookTargets[random].position;
			tempTarget = target;
        }
        else
        {
