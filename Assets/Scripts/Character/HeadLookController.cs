...
public class HeadLookController : MonoBehaviour {
    ...
    {
        int d = Mathf.RoundToInt(Time.time);
        var targetArray = GameObject.FindGameObjectsWithTag("GazeTarget");

        var random = new System.Random();

       target = targetArray[random.Next(targetArray.Length)].transform.position;
        tempTarget = target;
    }
